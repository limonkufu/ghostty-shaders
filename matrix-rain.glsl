...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Modified camera setup for flat projection
    vec3 ro = vec3(0.0, 0.0, 0.0);  // Camera at origin
    vec3 rd = vec3(uvInverted.x - 0.5, 1.0, uvInverted.y - 0.5);  // Orthographic projection
    rd = normalize(rd);
